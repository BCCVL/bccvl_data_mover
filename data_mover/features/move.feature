...
Scenario: Move http://www.intersect.org.au to test_machine
    ...
    And I want to move a file to "test_machine" with file path "/tmp/behave_test/bccvl/visualizer/some_file.txt"
    ...
    And I should see the file "/tmp/behave_test/bccvl/visualizer/some_file.txt"

Scenario: Move with invalid parameters
    Given I am connected to the Data Mover server
    And I want to move a file to "unknown_destination" with source path "/tmp/behave_test/bccvl/visualizer/some_file.txt"
...
Scenario: Move http://www.intersect.org.au to an unknown destination
    Given I am connected to the Data Mover server
    And I want to move a file to "unknown_destination" with file path "/tmp/behave_test/bccvl/visualizer/some_file.txt"
    And my file type is "url" with id "http://www.intersect.org.au"
    When I request a move with the defined requirements
...
Scenario: Move http://www.intersect.org.au to test_machine_bad - Unable to send to destination
    Given I am connected to the Data Mover server
    And I want to move a file to "test_machine_bad" with file path "/tmp/behave_test/bccvl/visualizer/some_file.txt"
    And my file type is "url" with id "http://www.intersect.org.au"
    When I request a move with the defined requirements
    Then I should see that the response is PENDING or IN_PROGRESS
    Then I wait 5 seconds
