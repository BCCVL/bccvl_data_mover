...
class TestAlaJobServices(unittest.TestCase):
    ...
    def testCreateNewJob(self):
        ...
        toTest._db_service.add.return_value = ALAJob(lsid, dataset_id)

        new_job = toTest.createNewJob(lsid, dataset_id)

        self.assertIsNotNone(new_job)
        self.assertEqual(lsid, new_job.lsid)
        self.assertEqual(dataset_id, new_job.dataset_id)
    ...
    def testFindById(self):
        ...
        toTest._db_service.findById.return_value = ALAJob(lsid, dataset_id)
    ...
    def testUpdate(self):
        ...
        updated_dataset_id = 431

        toTest = ALAJobService(None)
        toTest._db_service = MagicMock()
        toTest._db_service.update = MagicMock()
        toTest._db_service.update.return_value = ALAJob(updated_lsid, updated_dataset_id)

        new_job = ALAJob(lsid, dataset_id)
        new_job.lsid = updated_lsid
        new_job.dataset_id = updated_dataset_id

        out_job = toTest.update(new_job)

        self.assertIsNotNone(out_job)
        self.assertEqual(updated_lsid, out_job.lsid)
        self.assertEqual(updated_dataset_id, out_job.dataset_id)
    ...
    def testUpdateStatus(self):
        ...
        expected = ALAJob(lsid, dataset_id)
        ...
        self.assertEqual(expected.dataset_id, out_job.dataset_id)
